...
function sgdgc(opfunc, x, config, state)
   -- (0) get/update state
   local config = config or {}
   local state = state or config
   local lr = config.learningRate or 0.05
   ...
   local clip = config.clipval or 5
   ...
   -- (5) gradient clipping
   ndfdx = dfdx:norm()
   if clip and ndfdx >= clip then
      dfdx:div(ndfdx):mul(clip)
   end 
