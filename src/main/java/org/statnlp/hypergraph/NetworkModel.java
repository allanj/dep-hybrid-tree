...
//			print("Thread "+threadId+" has "+insts[threadId].length+" instances.", outstreams);
	...
	public void train(Instance[] trainInsts, int maxNumIterations) throws InterruptedException {
		train(trainInsts, maxNumIterations, null, null, -1, -1);
	}
	 ...
	 */
	public void train(Instance[] trainInstances, int maxNumIterations, Instance[] devInstances, Function<Instance[], Metric> evalFunction, int k, int epochLim) throws InterruptedException{
		...
		if(NetworkConfig.LOAD_PRETRAIN_WEIGHT) {
			double[] modelW = this._fm._param_g.getWeights();
			System.arraycopy(NetworkConfig.pretrainWeights, 0, modelW, 0, modelW.length);
		}
		...
		try{
			...
			for(int it = 0; it<=maxNumIterations; it++){
				...
				if (NetworkConfig.LOAD_PRETRAIN_WEIGHT && NetworkConfig.FIX_PRETRAIN_WEIGHT) {
					double[] modelW = this._fm._param_g.getWeights();
					System.arraycopy(NetworkConfig.pretrainWeights, 0, modelW, 0, modelW.length);
					Arrays.fill(this._fm._param_g._counts, 0.0);
				}
				...
				if (devInstances != null && evalFunction != null && k > 0 && (it + 1) % k == 0 && (epochNum + 1) >= epochLim) {
					/**
				...
				if(offset >= instIds.size()) {
					...
					if (NetworkConfig.SAVE_EPOCH_MODEL && (epochNum + 1) >= epochLim) {
						System.out.println("saving epoch model..");
						ObjectOutputStream oos;
						try {
							oos = RAWF.objectWriter(NetworkConfig.EPOCH_MODEL_FILE + epochNum);
							if (NetworkConfig.USE_NEURAL_FEATURES)
								this.getFeatureManager().getParam_G().getNNParamG().getAllNets().get(0).setModelFile(NetworkConfig.EPOCH_NN_MODEL_FILE + epochNum);
